...
/**
 * Returns the settings for the provided plugin
 * @param {string} [pluginName]
 */
Guild.prototype.getSettings = function (pluginName) {
    const settings = Settings.get(this);
    if (!pluginName) return settings;
    if (pluginName in settings.plugins) return settings.plugins[pluginName];
    this.client.logger.error(`getSettings: Settings for plugin "${pluginName}" not found`);
};

Guild.prototype.updateSettings = async function () {
    const settings = Settings.get(this);
    await settings.save();
};
...
Guild.prototype.getT = function (key, args) {
    const locale = this.getSettings("core").locale;
    const tFunction = this.client.translations.get(locale ?? this.client.defaultLanguage);
