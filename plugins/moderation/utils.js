...
const { addModLogToDb, getWarningLogs } = require("./schemas/ModLog");
...
const logModeration = async (issuer, target, reason, type, data = {}) => {
    ...
    switch (type.toUpperCase()) {
        ...
        case "TIMEOUT":
            embed.setColor(config.get("EMBED_COLORS").TIMEOUT);
            break;

        case "UNTIMEOUT":
            embed.setColor(config.get("EMBED_COLORS").UNTIMEOUT);
            break;

        case "KICK":
            embed.setColor(config.get("EMBED_COLORS").KICK);
            break;

        case "SOFTBAN":
            embed.setColor(config.get("EMBED_COLORS").SOFTBAN);
            break;

        case "BAN":
            embed.setColor(config.get("EMBED_COLORS").BAN);
            break;

        case "UNBAN":
            embed.setColor(config.get("EMBED_COLORS").UNBAN);
            break;

        case "VMUTE":
            embed.setColor(config.get("EMBED_COLORS").VMUTE);
            break;

        case "VUNMUTE":
            embed.setColor(config.get("EMBED_COLORS").VUNMUTE);
            break;

        case "DEAFEN":
            embed.setColor(config.get("EMBED_COLORS").DEAFEN);
            break;

        case "UNDEAFEN":
            embed.setColor(config.get("EMBED_COLORS").UNDEAFEN);
            break;

        case "DISCONNECT":
            embed.setColor(config.get("EMBED_COLORS").DISCONNECT);
            break;

        case "MOVE":
            embed.setColor(config.get("EMBED_COLORS").MOVE);
            break;
    }
...
module.exports = class ModUtils {
    ...
    static async warnTarget(issuer, target, reason) {
        ...
        try {
            const warnings = await getWarningLogs(issuer.guild.id, target.id);
            logModeration(issuer, target, reason, "Warn");
            let warningCount = warnings?.length || 0;
            warningCount += 1;
            const settings = issuer.guild.getSettings("moderation");
