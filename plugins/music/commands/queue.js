...
function getQueue({ client, guild }, pgNo) {
    ...
    if (!player) return guild.getT("music:COMMON.NO_MUSIC");

    const queue = player.queue;
    const embed = EmbedUtils.embed().setAuthor({
        name: guild.getT("music:QUEUE.EMBED_TITLE", {
            guild: guild.name,
        }),
    });
    ...
    if (queue.current)
        embed.addFields({
            name: guild.getT("music:QUEUE.CURRENT"),
            value: `[${queue.current.title}](${queue.current.uri})`,
        });
    if (!tracks.length) {
        embed.setDescription(
            page > 1
                ? guild.getT("music:QUEUE.NO_TRACKS_IN_PAGE")
                : guild.getT("music:QUEUE.NO_TRACKS"),
        );
    } else {
        embed.setDescription(
